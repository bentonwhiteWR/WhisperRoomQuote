...
const SECTIONS = [
  // Cover / Series / EFP files are named with the size+variant stem, NOT the
  // full "MDL " prefix. Example from EFPManuals: "4848 S EFP.pdf". So match
  // on ctx.modelStem ("4848 S") rather than ctx.model ("MDL 4848 S").
  // (EFP additionally falls back to size-only — see its row below.)
  { order: 'A',  folder: 'CoverManuals',       match: ctx => ctx.modelStem                                        },
  ...
  // (Section F — Overseas — retired. The "Overseas" checkbox is no longer
  // shown on the modal; leaving the section out so it can't be re-enabled
  // accidentally.)
  ...
  { order: 'J',  folder: 'SeriesManuals',      match: ctx => ctx.modelStem                                        },
  // Vent: K = base, L = upgrades. Both are non-LP unless model contains "LP".
  // Skip both if (a) RM is checked (section M takes over) or (b) the model
  // is an SNV/ENV no-vent variant.
  { order: 'K',  folder: 'VentilationManuals', match: ctx => ctx.isLP ? 'LP Vent system'   : 'Ventilation System',   when: (o, ctx) => !o.rm && !ctx.isNV },
  { order: 'L',  folder: 'VentilationManuals', match: ctx => ctx.isLP ? 'LP Vent Upgrades' : 'Ventilation Upgrades', when: (o, ctx) => !o.rm && !ctx.isNV },
  ...
  // EFP files are saved per-size under whichever vent variant happens to
  // exist (e.g. only "8484 E EFP.pdf", no "8484 S"). Try the exact
  // "size + variant" stem first, then fall back to size-only so an S booth
  // still finds the E file (and vice-versa) when only one variant is on file.
  { order: 'Q',  folder: 'EFPManuals',         match: ctx => [ctx.modelStem, ctx.size], when: o => !!o.efp       },
...
// Strip the "MDL " prefix AND normalize the no-vent suffix variants so we
// can match against filename conventions that use just the size+variant.
//   "MDL 4848 S"   → "4848 S"
//   "MDL 4848 SNV" → "4848 S"   (no dedicated SNV manuals — same as S)
//   "MDL 4848 ENV" → "4848 E"   (no dedicated ENV manuals — same as E)
//   "MDL 127 LP S" → "127 LP S"
// The "no ventilation" variants share Cover / Series / EFP / etc with their
// vented kin; the only difference is that the K and L ventilation sections
// get skipped — see `isNV` below + the K/L `when` gates.
function _stripMdlPrefix(model) {
  const stem = String(model || '').replace(/^\s*MDL\s+/i, '').trim();
  return stem.replace(/\bSNV\b/i, 'S').replace(/\bENV\b/i, 'E');
}

// True for SNV/ENV (no-ventilation) variants — skip the K/L sections.
function _isNV(model) {
  return /\b(SNV|ENV)\b/i.test(String(model || ''));
}
...
function planSections(opts = {}) {
  // No server-side ADA cascade here — the rep's checkbox state on the
  // modal is the source of truth. WA UPG quotes legitimately want the
  // ADA door without Ramp/EFP, so a blanket `ada → efp+ramp` cascade
  // would over-include for them. The frontend pre-fill handles the
  // distinction (only "ADA *" line items trigger the cascade; "WA UPG"
  // ones don't).
  const ctx = {
    model:      String(opts.model || '').trim(),
    modelStem:  _stripMdlPrefix(opts.model),
    adaSize:    String(opts.adaSize || '').trim(),
    size:       _extractSize(opts.model),
    isLP:       _isLP(opts.model),
    isNV:       _isNV(opts.model),
  };
  const plan = [];
  for (const s of SECTIONS) {
    if (s.when && !s.when(opts, ctx)) continue;
    const matched = typeof s.match === 'function' ? s.match(ctx) : s.match;
    // A section may supply several candidate needles to try in order (EFP:
    // exact "size + variant" first, then size-only fallback).
    const needles = (Array.isArray(matched) ? matched : [matched]).filter(Boolean);
    if (!needles.length) continue;
    plan.push({ order: s.order, folder: s.folder, needle: needles[0], needles });
  }
...
async function buildAssemblyManual(opts = {}) {
  ...
  for (const s of plan) {
    ...
    let file = null;
    try {
      // Try each candidate needle in order; first file wins.
      for (const nd of (s.needles && s.needles.length ? s.needles : [s.needle])) {
        file = await _findFileByMatch(s.folder, nd);
        if (file) break;
      }
    } catch (e) {
