...
<div class="main">
  ...
  ${[
    ...
    {
      v:'1.22.1', date:'May 15, 2026', tag:'fix',
      changes:[
        {t:'fix', d:'**FL discretionary surtax cap — manual application.** v1.22.0 wired up TaxJar\'s line-item mode on the assumption that TaxJar would auto-apply state-specific per-item caps. Verified today against the same Pinellas County order (W-1105142605): even with all 5 line items sent properly, TaxJar still returned the uncapped $558.93 — it does NOT auto-apply FL\'s $5,000 surtax cap. Caught it by inspecting the /api/tax response: `tax: 558.93, mode: "line_items"`. So we now apply the cap ourselves on the server side, using the state/local rate split TaxJar provides. For any FL request in line-item mode: state portion (6%) applies to the full per-line taxable amount, county/special-district surtax (1% here) applies only to the first $5,000 of each individual item. Per-unit post-discount price is computed, capped at $5k, multiplied by quantity (FL cap is per-item, so qty=2 of an over-$5k item gets the cap twice). Components rounded per-jurisdiction before summing to match QB\'s invoice math to the cent. Triggers only when `to_state === FL` AND line-item mode AND local rate > 0. Logs `[tax] FL §212.054 surtax cap applied: $X → $Y (saved $Z)` when it kicks in. Other states with per-item caps (TN single-article, etc.) would each need a similar block — punted until surfaced by a real order.'},
      ]
    },
    {
      v:'1.22.0', date:'May 15, 2026', tag:'fix',
      changes:[
        {t:'fix', d:'**TaxJar line-item mode — quote-builder tax now matches QuickBooks tax on the penny, across every US jurisdiction with per-item rules.** Background: a Florida (Pinellas County) order with an MDL 4848 S over $5k showed $558.93 sales tax on the quote but QB invoiced $552.00. The $6.93 gap was Florida\'s discretionary surtax cap — county 1% surtax only applies to the first $5,000 of each tangible item (FL Stat. §212.054(2)(b)). QB\'s AST handles this natively because it sees line items; our quote builder was POSTing TaxJar a single aggregate `subtotal` (post-discount), so TaxJar could only return a flat blended rate (7%) and we\'d multiply it against the whole basis — over-charging on the $695.25 above the cap. Fix: client (quote-builder.html `getTax()`) now builds a `lineItems[]` array from the quote\'s positive-priced lines, allocates the order-level discount per line by share of positive subtotal (works for both % and $ discounts), and sends it to /api/tax. Server (`lib/taxjar.js` `calculateTaxProper`) accepts the array and forwards it as TaxJar\'s `line_items[]` body. TaxJar then applies every state-specific per-item rule natively — FL\'s $5k cap, TN\'s single-article local tax cap, MA\'s $175 clothing exemption, NY\'s apparel rules, ~50 other quirks — no per-state code on our side. Effective rate is back-computed from the actual returned amount ÷ taxable basis, so client-side live recompute (`getTaxAmount` does rate × base on every product/freight/discount change) lands on the same penny TaxJar returned. Headline rate is also returned for display. Aggregate-amount mode kept as fallback for any caller that doesn\'t send line items. Credits (negative-price lines) are excluded from the TaxJar request — if a credit materially affects the taxable total, use the tax-override field.'},
      ]
    },
    {
