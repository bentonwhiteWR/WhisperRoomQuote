...
function planSections(opts = {}) {
  // Server-side cascade: when ADA is on, force EFP + Ramp on too. The
  // frontend modal already pre-ticks these checkboxes, but enforcing
  // it here means a direct API caller (or a rep who accidentally
  // unticks one) still gets the right sections.
  if (opts.ada) opts = { ...opts, efp: true, ramp: true };
  const ctx = {
