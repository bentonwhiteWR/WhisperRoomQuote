...
let _writelog;
let _puppeteer;
let _withBrowser;   // shared, semaphore-guarded Chromium runner from lib/pdf.js
...
function init(deps) {
  ...
  _withBrowser  = deps.withBrowser;   // optional; falls back to a standalone launch
}
...
// Pallet weight deducted per-pallet before sending to ABF. Our stored
// per-pallet weight is the gross weight rep sees on the floor (booth +
// accessories + the wooden pallet itself). ABF rates off product weight
// only — they add the pallet weight on their side. Without this
// subtraction we over-report by ~144 lbs per pallet and ABF returns a
// higher rate than its own public quote page. OD is the inverse case:
// our stored weight is the same (gross), but OD wants gross including
// the pallet, and our adjustment for that lives at /api/orders-freight.
const ABF_PALLET_DEDUCT_LBS = 144;
...
async function fetchAndCacheTracking(trackingNumber, carrier) {
  ...
  if (['UPS','FEDEX','USPS'].includes(carrierUpper)) {
    ...
    // The actual scrape, given a launched browser. Shared by the
    // semaphore-guarded path and the legacy fallback.
    ...
    try {
      ...
      if (_withBrowser) {
        // Tracking is background/non-urgent → share the single-flight renderer
        // with PDF generation so we never launch a second Chromium at once
        // (the cause of "posix_spawn … Resource temporarily unavailable").
        // Wait only briefly; if a user PDF job holds the renderer, skip this
        // shipment and let the next 30-min poll retry it.
        return await _withBrowser({ maxWaitMs: 20000 }, scrape);
      }
      // Fallback: withBrowser not wired — legacy standalone launch.
...
function buildAbfUrl(pallets, totalWeight, consCity, consState, consZip, isCanadian, accessories, servType, pickupDate) {
  // pickupDate: optional YYYY-MM-DD. ABF's TRDAYS / DUEDATE depend on the
  // ShipMonth/Day/Year we send, so passing a future pickup matters for
  // transit-day rendering. Parse the date parts manually rather than
  // `new Date(pickupDate)` to avoid TZ-driven off-by-one (a bare
  // YYYY-MM-DD is parsed as UTC midnight, which slides to the prior day
  // in any negative UTC offset).
  let pickupMonth, pickupDay, pickupYear;
  if (pickupDate && /^\d{4}-\d{2}-\d{2}$/.test(pickupDate)) {
    const [y, m, d] = pickupDate.split('-').map(Number);
    pickupYear = y; pickupMonth = m; pickupDay = d;
  } else {
    const today = new Date();
    pickupYear  = today.getFullYear();
    pickupMonth = today.getMonth() + 1;
    pickupDay   = today.getDate();
  }
  ...
  // ConsCity / ConsState are optional — ABF geocodes from ConsZip when blank,
  // so reps can run a quote with just the destination ZIP. Sending empty
  // params (`ConsCity=&ConsState=`) made ABF reject the request, so omit
  // them entirely when not provided.
  if (consCity) parts.push(`ConsCity=${encodeURIComponent(consCity)}`);
  if (consState) parts.push(`ConsState=${consState}`);
  parts.push(
  ...
  pallets.forEach((pl, i) => {
    const n = i + 1;
    const grossLbs = parseFloat(pl.weight) || 0;
    // Strip the pallet's own wood weight — see ABF_PALLET_DEDUCT_LBS
    // at top of file. Floor at 0 so an empty / very-light pallet doesn't
    // send a negative to ABF (which would 400 the request).
    const abfLbs = Math.max(0, Math.round(grossLbs - ABF_PALLET_DEDUCT_LBS));
    parts.push(
      `FrtLng${n}=${pl.l}`, `FrtWdth${n}=${pl.w}`, `FrtHght${n}=${pl.h}`,
      `UnitType${n}=PLT`, `Wgt${n}=${abfLbs}`, `UnitNo${n}=1`,
      `Class${n}=${FREIGHT_CLASS}`, `NMFCItem${n}=${nmfcItem}`, `NMFCSub${n}=${nmfcSub}`
    );
  });
  parts.push('ShipAff=Y', `ShipMonth=${pickupMonth}`, `ShipDay=${pickupDay}`, `ShipYear=${pickupYear}`);
...
function parseAbfXml(xmlText) {
  ...
  // Notes: ABF's API surfaces service-level messages alongside the rate
  // (e.g. "Delivery is only available on Tuesday, Wednesday, and
  // Thursday." for destinations with restricted delivery days). The
  // exact element name varies across ABF's API surface — try a list of
  // candidates and dedupe. When a delivery restriction is missed in the
  // future, check the raw XML log line below for the actual element
  // name and add it to noteTags.
  const notes = [];
  const seen = new Set();
  const pushNote = (txt) => {
    if (!txt) return;
  ...
  const noteTags = [
    'NOTE', 'NOTES', 'MESSAGE', 'MESSAGES', 'INFO', 'SERVICEMSG',
    'SERVICEMESSAGE', 'SERVICENOTE', 'COMMENT', 'COMMENTS',
    'DELIVERYNOTE', 'DELIVERYINFO', 'DELIVERYDAYS', 'WARNING'
  ];
  noteTags.forEach(tag => {
    const re = new RegExp(`<${tag}[^>]*>([\\s\\S]*?)<\\/${tag}>`, 'gi');
    let n;
    while ((n = re.exec(xmlText)) !== null) pushNote(n[1].replace(/<[^>]+>/g, ''));
  });
  // Also catch ITEM elements that carry a NOTE/MESSAGE rather than a
  // rate (no AMOUNT attribute, FOR is descriptive). The earlier rate
  // loop only matches ITEMs with both FOR= and AMOUNT=.
  const itemNoteRe = /<ITEM[^>]+FOR="([^"]+)"[^>]*>([\s\S]*?)<\/ITEM>/gi;
  let in_;
  while ((in_ = itemNoteRe.exec(xmlText)) !== null) {
    const forAttr = (in_[1] || '').toUpperCase();
    if (/NOTE|MESSAGE|DELIVERY|WARNING|INFO|SERVICE/.test(forAttr)) {
      pushNote(in_[2].replace(/<[^>]+>/g, ''));
    }
  }
  // Log notes when present so we can confirm the parser caught them.
  if (notes.length) console.log(`[ABF] notes: ${notes.join(' | ')}`);

  // Quote ID — used to deep-link to ArcBest's web rate-quote page so
  // the rep can see ABF's full rendering. Confirmed element name from
  // the official ArcBest API docs: <QUOTEID>. Public URL format:
  // https://arcb.com/tools/rate-quote.html#/LTLX8W1316
  let quoteId = null;
  const qm = xmlText.match(/<QUOTEID[^>]*>([^<]+)<\/QUOTEID>/i);
  if (qm && qm[1].trim()) quoteId = qm[1].trim();

  // Guaranteed Transit Options. Per the docs, ABF returns these in
  // the same response as the standard rate when guaranteed service is
  // available for the lane. Each <OPTION> inside <GUARANTEEDOPTIONS>
  // has GUARANTEEDCHARGE / GUARANTEEDDELDATE / GUARANTEEDBYTIME. The
  // caller renders one extra rate card per option.
  const guaranteedOptions = [];
  const goBlock = xmlText.match(/<GUARANTEEDOPTIONS[\s\S]*?<\/GUARANTEEDOPTIONS>/i);
  if (goBlock) {
    const optRe = /<OPTION[\s\S]*?<\/OPTION>/gi;
    let opt;
    while ((opt = optRe.exec(goBlock[0])) !== null) {
      const block = opt[0];
      const charge   = parseFloat((block.match(/<GUARANTEEDCHARGE[^>]*>([^<]+)<\/GUARANTEEDCHARGE>/i)?.[1] || '').trim()) || 0;
      const delDate  = (block.match(/<GUARANTEEDDELDATE[^>]*>([^<]+)<\/GUARANTEEDDELDATE>/i)?.[1] || '').trim();
      const byTime   = (block.match(/<GUARANTEEDBYTIME[^>]*>([^<]+)<\/GUARANTEEDBYTIME>/i)?.[1] || '').trim();
      if (charge > 0) {
        guaranteedOptions.push({ charge: Math.round(charge * 100) / 100, delDate, byTime });
      }
    }
  }
  if (guaranteedOptions.length) console.log(`[ABF] guaranteed options (${guaranteedOptions.length}):`, guaranteedOptions.map(o => `$${o.charge} by ${o.byTime}`).join(', '));

  if (cost === 0) {
    console.log(`[ABF] zero-cost response (truncated): ${String(xmlText).slice(0, 1500)}`);
    throw new Error('No rate returned from ABF — please verify the destination ZIP, city, and state are correct');
  }
  return { cost: Math.round(cost * 100) / 100, dynDisc, transit, notes, quoteId, guaranteedOptions };
}
