...
- **PDF generation is single-threaded** (Puppeteer semaphore `_pdfBusy` in `lib/pdf.js`). As of v1.58.1 BOTH paths share it: `generatePdf` (download, waits ≤30s) and `generatePdfBuffer` (Drive uploads — invoices/quotes/orders/vendor POs, waits ≤60s). Before v1.58.1 the buffer path bypassed the gate, so concurrent Drive PDFs launched multiple Chromiums and Railway threw "Failed to launch the browser process … Cannot fork … Resource temporarily unavailable". Don't route any new PDF path around `_acquirePdf`/`_releasePdf`. Launches also retry 3× (`_launchWithRetry`) for transient fork failures. **v1.61.1:** if "Cannot fork" returns even when serialized, it's the Railway container hitting its memory/PID ceiling, not a code race. Mitigations in place: `PUPPETEER_EXECUTABLE_PATH=/usr/lib/chromium/chromium` (the real binary — `/usr/bin/chromium` is a shell wrapper that forks to source `/etc/chromium.d/*` on every launch; Dockerfile asserts the path at build), and `_BASE_LAUNCH_ARGS` includes `--disable-crash-reporter --disable-breakpad` so Chromium doesn't spawn the crashpad handler (the subprocess seen failing). The durable fix is **more Railway memory**; next code lever is reusing one long-lived browser instead of launch-per-PDF.
- **Top navbar is shared (v1.59.0+):** the dashboard nav lives in `assets/nav.js` (served at `/assets/nav.js`, included via `<script src="/assets/nav.js" defer>`). It renders the standard links into each page's `.topbar-nav`, removing any hard-coded `a.nav-link` first and highlighting the active page by path. To add/rename/reorder a nav item, edit the `LINKS` array in that one file. quote-builder uses its own reduced `topbar-nav-btn` bar (not shared). "Suppliers" (Audimute POs) is no longer a top-level item — it's a subtab under Vendor Hub (`/vendor-pos` ↔ `/suppliers` cross-link); Vendor Hub stays highlighted on both.
