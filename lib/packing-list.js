      ...
      if (hasHX && (hasWA || hasADA)) {
        ...
        if (swap) {
          ...
          for (const row of rows) {
            ...
            if (removed && removed.eachLb != null) netLb = (netLb || 0) - removed.eachLb;
            const c = COMPONENTS[row.add] || {};
            const eachLb = (c.lb == null) ? null : Number(c.lb);
            if (eachLb != null) netLb = (netLb || 0) + eachLb;
            lines.push({ code: row.add, desc: c.desc || '(unknown component)', pack: c.pack || '', eachLb,
              L: c.L ?? null, W: c.W ?? null, T: c.T ?? null, known: !!COMPONENTS[row.add], addedFrom: 'HX+WA' });
          }
        }
      }
