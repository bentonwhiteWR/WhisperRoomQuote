...
**Most recent shipped:** v1.9.14 — OD rate requests now add 140 lbs per pallet to the per-item weight (OD prices off gross weight inc. pallet; ABF unchanged). Combined with v1.9.12 + v1.9.13, OD rates should now match OD's own page. (Previous: v1.9.13 OD NMFC; v1.9.12 OD double-count fix + clipboard; v1.9.11 OD reference number display.)
...
- v1.9.14 on staging awaiting test. Prod (main) at v1.9.13.
