...
function parseAbfXml(xmlText) {
  ...
  // the rep can see ABF's full rendering (including service messages
  // like restricted delivery days that aren't in this XML response).
  // Public URL format: https://arcb.com/tools/rate-quote.html#/LTLX8W1316
  // The exact XML element name varies — try several candidates and log
  // them all so a missed shape can be added.
  let quoteId = null;
  const idTags = ['RATEQUOTENUMBER', 'QUOTENUMBER', 'QUOTEID', 'QUOTEREF', 'RATEQUOTEID', 'RESPID', 'QUOTERESPID', 'QUOTEREFNUM', 'CONFIRMATIONNUMBER', 'CONFIRMATION'];
  const idHits = {};
  idTags.forEach(tag => {
    const re = new RegExp(`<${tag}[^>]*>([^<]+)</${tag}>`, 'i');
    const x = xmlText.match(re);
    if (x && x[1].trim()) { idHits[tag] = x[1].trim(); if (!quoteId) quoteId = x[1].trim(); }
  });
  // Also catch attribute-style: <SOMETHING ID="LTLX..."/>
  if (!quoteId) {
    const attrRe = /\b(?:RATEQUOTENUMBER|QUOTENUMBER|QUOTEID|QUOTEREF|RATEQUOTEID|RESPID)="([^"]+)"/i;
    const am = xmlText.match(attrRe);
    if (am && am[1].trim()) { idHits['ATTR'] = am[1].trim(); quoteId = am[1].trim(); }
  }
  if (Object.keys(idHits).length) console.log(`[ABF] quote-id candidates:`, idHits);
  ...
  return { cost: Math.round(cost * 100) / 100, dynDisc, transit, notes, quoteId };
}
