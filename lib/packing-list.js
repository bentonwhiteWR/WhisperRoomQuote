...
let ADA_BY_MODEL = {};          // model → { add:{...}, remove:{...} } — full ADA package per booth
let _meta = {};
...
const FEATURE_SUBS = [
  ...
  { name: 'ADA', once: true,
    ...
    resolve: (name, _lineQty, ctx) => {
      const rec = ADA_BY_MODEL[featureModelKey(ctx.bomKey)];
...
function generate(lineItems, opts = {}) {
  ...
  // Box count = total PL rows (each row = one numbered box on the PL) × booth qty,
  // summed across booths. Hardware Box # = the 1-based row number of the booth
  // hardware kit (the HDWR <model> pack, not HDWR EXP) in the FIRST booth's PL —
  // the PL's "#" column is the box numbering, and the rows are code-sorted.
  let boxCount = 0;
  out.forEach(r => { r.boxCount = (r.lines || []).length; boxCount += r.boxCount * (r.qty || 1); });
  let hardwareBox = null;
  const firstRoom = out[0];
  ...
  return {
    rooms: out,
    orphanFeatures,
    totals: { rooms: out.length, netLb: Math.round(grandNet * 100) / 100, boxCount, hardwareBox },
    defaults: DEFAULTS,
