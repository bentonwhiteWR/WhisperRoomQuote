...
// Returns raw query body (including QueryResponse envelope) for diagnostic use
...
async function createInvoice({ customerRef, docNumber, txnDate, lines, memo, billAddr, shipAddr, globalTaxCalc }) {
  const { accessToken, realmId } = await getAccessToken();
  const payload = {
