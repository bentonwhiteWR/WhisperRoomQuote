...
async function createInvoiceForQuote({ quoteNumber, dealId, customer, lineItems, daysUntilDue, expectedTotalCents, discountPct = 0, allowCC = true, allowACH = true, allowWire = true }) {
  ...
  // If the quote has a global discount, create a one-shot Stripe Coupon and
  // apply it at the invoice level. Items with `lineDiscount > 0` (product
  // lines) stay discountable; freight/tax/install (lineDiscount = 0) are
  // flagged `discountable: false` so the coupon only hits product lines —
  // matches HubSpot's per-line `hs_discount_percentage` scope. Stripe's
  // hosted invoice renders this as a separate "Discount" row under the
  // subtotal instead of baked into each line.
  const pct = Math.max(0, Math.min(100, parseFloat(discountPct || 0)));
  let couponId = null;
  if (pct > 0) {
    const pctDisplay = pct.toFixed(2).replace(/\.?0+$/, '');
    const coupon = await stripeReq('/coupons', {
      percent_off:     String(pct),
      duration:        'once',
      max_redemptions: '1',
      name:            `${pctDisplay}% Off${quoteNumber ? ` — Quote ${quoteNumber}` : ''}`.slice(0, 40),
      metadata: {
  ...
  // Payment method gating. Per-quote flags from the rep UI:
  //   allowCC   → 'card'              (~3.4% fee)
  //   allowACH  → 'us_bank_account'   (0.8%, cap $5; 4–5 day clearing)
  //   allowWire → 'customer_balance'  ($8 flat; 1–2 day clearing; effectively
  //                                    irreversible — preferred for $25k+)
  // Stripe REQUIRES payment_method_options.customer_balance.bank_transfer when
  // customer_balance is in the list — invoice creation 400s without it.
  //
  // Wire also requires a customer name (Stripe customer_balance rejects nameless
  // customers). If wire is requested but no name on the customer record, soft-
  // drop wire from the list and log — don't 400 the whole invoice.
  const stripeCustomerName = stripeCustomer.name || (customer && (customer.name || [customer.firstName, customer.lastName].filter(Boolean).join(' ').trim())) || '';
  let wireOn = !!allowWire;
  if (wireOn && !stripeCustomerName) {
    _writelog('warn', 'stripe.invoice.wire-dropped', `Wire transfer requested but customer has no name — dropping customer_balance from payment methods`, { quoteNum: quoteNumber || null, dealId: dealId || null });
    wireOn = false;
  }
  const paymentMethodTypes = [];
  if (allowCC)   paymentMethodTypes.push('card');
  if (allowACH)  paymentMethodTypes.push('us_bank_account');
  if (wireOn)    paymentMethodTypes.push('customer_balance');
  if (!paymentMethodTypes.length) {
    throw new Error('No payment methods enabled for this invoice (allowCC, allowACH, allowWire all false)');
  }

  // ACH takes 4–5 business days to clear; a 7-day due window means legit ACH
  // payers see "past due" reminders before the bank confirms. Stretch to 14
  // when ACH is enabled. Explicit caller override still wins.
  const effectiveDue = (typeof daysUntilDue === 'number')
    ? daysUntilDue
    : (allowACH ? 14 : 7);

  const paymentSettings = { payment_method_types: paymentMethodTypes };
  if (wireOn) {
  ...
  const draftParams = {
    ...
    days_until_due:                 String(effectiveDue),
    auto_advance:                   'false',
    pending_invoice_items_behavior: 'exclude',
    description:                    `WhisperRoom Order — Quote ${quoteNumber || ''}`.trim(),
    footer:                         'Thank you for choosing WhisperRoom. Questions about this invoice? Reply to the email it came with, or contact your sales rep directly. We appreciate your business.',
  ...
  const customFields = [];
  if (quoteNumber) customFields.push({ name: 'Quote Number', value: String(quoteNumber).slice(0, 30) });
  if (dealId)      customFields.push({ name: 'Deal ID',      value: String(dealId).slice(0, 30) });
  if (customFields.length) draftParams.custom_fields = customFields;
  if (couponId)            draftParams.discounts     = [{ coupon: couponId }];

  const draft = await stripeReq('/invoices', draftParams);
  ...
  //
  // The quote-level coupon (applied above on the invoice) hits any invoiceitem
  // where `discountable !== false`. Items with `lineDiscount > 0` (product
  // lines from /api/create-invoice) stay default-discountable; freight/tax/
  // install lines carry `lineDiscount = 0` and are flagged non-discountable
  // here so the coupon does not also discount them.
  ...
  for (const item of itemsToCharge) {
    ...
    if (!(parseFloat(item.lineDiscount || 0) > 0)) {
      // Freight, tax, install — coupon should not discount these.
      itemParams.discountable = 'false';
    }
    await stripeReq('/invoiceitems', itemParams);
  }
