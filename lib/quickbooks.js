...
// ── Activity finder (audit-log-style "what changed when") ─────────
// IMPORTANT: QuickBooks Online does NOT expose its native Audit Log via the
// public API (the "who" / before-after is UI-only). The best the API offers is
// querying entities by MetaData.LastUpdatedTime — so this returns WHAT changed
// and WHEN (with a deep link), but never WHO. Per-change history isn't available
// either; we see each entity's latest state + last-modified time. Deletes are
// not returned by a query (would need CDC) — noted to the user in the UI.
//
// Returns { rows, counts, capped, errors } where each row is
//   { type, id, docNumber, name, amount, txnDate, created, lastUpdated, action, link }
const _QB_ACTIVITY_TYPES = {
  Invoice:       { path: 'invoice',        idp: 'txnId',  ref: 'CustomerRef' },
  Payment:       { path: 'recvpayment',    idp: 'txnId',  ref: 'CustomerRef' },
  CreditMemo:    { path: 'creditmemo',     idp: 'txnId',  ref: 'CustomerRef' },
  SalesReceipt:  { path: 'salesreceipt',   idp: 'txnId',  ref: 'CustomerRef' },
  RefundReceipt: { path: 'refundreceipt',  idp: 'txnId',  ref: 'CustomerRef' },
  Estimate:      { path: 'estimate',       idp: 'txnId',  ref: 'CustomerRef' },
  Bill:          { path: 'bill',           idp: 'txnId',  ref: 'VendorRef' },
  BillPayment:   { path: 'billpayment',    idp: 'txnId',  ref: 'VendorRef' },
  VendorCredit:  { path: 'vendorcredit',   idp: 'txnId',  ref: 'VendorRef' },
  Purchase:      { path: 'expense',        idp: 'txnId',  ref: 'EntityRef' }, // path overridden for checks
  JournalEntry:  { path: 'journal',        idp: 'txnId',  ref: null },
  Deposit:       { path: 'deposit',        idp: 'txnId',  ref: null },
  Transfer:      { path: 'transfer',       idp: 'txnId',  ref: null },
  Customer:      { path: 'customerdetail', idp: 'nameId', ref: null, nameField: 'DisplayName', noTxn: true },
  Vendor:        { path: 'vendordetail',   idp: 'nameId', ref: null, nameField: 'DisplayName', noTxn: true },
};
const QB_ACTIVITY_ENTITIES = Object.keys(_QB_ACTIVITY_TYPES);

function _qbActivityLink(type, row) {
  const cfg = _QB_ACTIVITY_TYPES[type];
  if (!cfg || !row.Id) return null;
  let p = cfg.path;
  if (type === 'Purchase') p = (row.PaymentType === 'Check') ? 'check' : 'expense';
  return `https://app.qbo.intuit.com/app/${p}?${cfg.idp}=${encodeURIComponent(row.Id)}`;
}
function _qbActivityName(type, row) {
  const cfg = _QB_ACTIVITY_TYPES[type];
  if (cfg.nameField) return row[cfg.nameField] || '';
  if (cfg.ref && row[cfg.ref]) return row[cfg.ref].name || row[cfg.ref].value || '';
  return (row.PrivateNote || '').slice(0, 60);
}

// Email-capable transaction types (carry EmailStatus + DeliveryInfo).
const _QB_EMAILABLE = new Set(['Invoice', 'Estimate', 'SalesReceipt', 'CreditMemo', 'RefundReceipt']);

// Light status from the entity's balance / due date (Invoice + Bill only).
function _qbActivityStatus(type, row) {
  const bal = row.Balance != null ? Number(row.Balance) : null;
  if (bal == null || (type !== 'Invoice' && type !== 'Bill')) return null;
  if (bal <= 0.0001) return 'Paid';
  if (row.DueDate && row.DueDate < new Date().toISOString().slice(0, 10)) return 'Overdue';
  return 'Open';
}

// QBO Change Data Capture — the only API surface that reports deletes. Limited
// to a ~30-day changedSince window. Returns the raw CDC envelope.
async function qbCdc(entities, changedSinceIso) {
  const { accessToken, realmId } = await getAccessToken();
  const ents = encodeURIComponent(entities.join(','));
  const r = await _httpsRequest({
    hostname: QB_API_HOST,
    path: `/v3/company/${realmId}/cdc?entities=${ents}&changedSince=${encodeURIComponent(changedSinceIso)}&minorversion=65`,
    method: 'GET',
    headers: { 'Authorization': `Bearer ${accessToken}`, 'Accept': 'application/json' },
  });
  if (r.status >= 400) throw new Error(`QB CDC ${r.status}: ${JSON.stringify(r.body).slice(0, 200)}`);
  return r.body;
}

// Deleted records via CDC. Only attempted when the range start is within the
// last 30 days (CDC's hard limit); older ranges return { skipped:true }.
async function _fetchDeletes(list, fromIso, toIso) {
  if (new Date(fromIso).getTime() < Date.now() - 30 * 86400000) return { deletes: [], skipped: true };
  let body;
  try { body = await qbCdc(list, fromIso); }
  catch (e) { return { deletes: [], error: e.message }; }
  const out = [];
  const groups = body?.CDCResponse?.[0]?.QueryResponse || [];
  for (const grp of groups) {
    for (const type of Object.keys(grp)) {
      if (!_QB_ACTIVITY_TYPES[type] || !Array.isArray(grp[type])) continue;
      for (const it of grp[type]) {
        if (String(it.status) !== 'Deleted') continue;          // CDC flags removals here
        const at = it.MetaData?.LastUpdatedTime || null;
        if (at && at > toIso) continue;                         // best-effort high bound
        out.push({
          type, id: it.Id, docNumber: it.DocNumber || '', name: '', amount: null,
          txnDate: null, created: null, lastUpdated: at, eventAt: at || toIso,
          action: 'deleted', link: null,
        });
      }
    }
  }
...
// Attachment-added events via the Attachable entity (separate from the txn it
// hangs off of, so it never shows up in a normal entity query).
async function _fetchAttachments(fromIso, toIso) {
  const out = [];
  try {
    let startPos = 1;
    for (let page = 0; page < 3; page++) {
      const qoql = `SELECT * FROM Attachable WHERE MetaData.LastUpdatedTime >= '${fromIso}' AND MetaData.LastUpdatedTime <= '${toIso}' ORDERBY MetaData.LastUpdatedTime STARTPOSITION ${startPos} MAXRESULTS 1000`;
      const data = await qbQuery(qoql);
      const rows = data?.QueryResponse?.Attachable || [];
      for (const row of rows) {
        const created = row.MetaData?.CreateTime || null;
        const upd = row.MetaData?.LastUpdatedTime || null;
        const ref = (row.AttachableRef || []).find(r => r.EntityRef && r.EntityRef.value);
        const attachedType = ref?.EntityRef?.type || null;
        const link = attachedType ? _qbActivityLink(attachedType, { Id: ref.EntityRef.value }) : null;
        out.push({
          type: 'Attachment', id: row.Id, docNumber: '',
          name: row.FileName || row.Note || '(attachment)', amount: null, txnDate: null,
          created, lastUpdated: upd, eventAt: created || upd,
          action: 'attachment', link, attachedTo: attachedType,
        });
      }
      if (rows.length < 1000) break;
      startPos += 1000;
    }
  } catch (e) { return { attachments: [], error: e.message }; }
  return { attachments: out };
}

async function fetchActivity({ fromIso, toIso, entities } = {}) {
  const list = (entities && entities.length ? entities : QB_ACTIVITY_ENTITIES)
    .filter(e => _QB_ACTIVITY_TYPES[e]);
  const PAGE = 1000, MAX_PAGES = 5;
  const counts = {}, capped = [], errors = [], notes = [];

  const perEntity = await Promise.all(list.map(async (type) => {
    const out = [];
    try {
      let startPos = 1;
      for (let page = 0; page < MAX_PAGES; page++) {
        const qoql = `SELECT * FROM ${type} WHERE MetaData.LastUpdatedTime >= '${fromIso}' AND MetaData.LastUpdatedTime <= '${toIso}' ORDERBY MetaData.LastUpdatedTime STARTPOSITION ${startPos} MAXRESULTS ${PAGE}`;
        const data = await qbQuery(qoql);
        const rows = data?.QueryResponse?.[type] || [];
        for (const row of rows) {
          const created = row.MetaData?.CreateTime || null;
          const lastUpdated = row.MetaData?.LastUpdatedTime || null;
          const emailed = _QB_EMAILABLE.has(type) && row.EmailStatus === 'EmailSent';
          const base = {
            type,
            id: row.Id,
            docNumber: row.DocNumber || row.PaymentRefNum || '',
            name: _qbActivityName(type, row),
            amount: _QB_ACTIVITY_TYPES[type].noTxn ? null : (row.TotalAmt != null ? Number(row.TotalAmt) : null),
            txnDate: row.TxnDate || null,
            created,
            lastUpdated,
            eventAt: lastUpdated,
            action: (created && created >= fromIso) ? 'created' : 'updated',
            link: _qbActivityLink(type, row),
            // Tier 1 enrichment — all from the SELECT * we already pull:
            syncToken: row.SyncToken != null ? Number(row.SyncToken) : null,  // edits since create
            status: _qbActivityStatus(type, row),
            balance: row.Balance != null ? Number(row.Balance) : null,
            memo: ((row.PrivateNote || row.CustomerMemo?.value || '').trim()).slice(0, 80) || null,
            linked: (row.LinkedTxn || []).map(l => ({ type: l.TxnType, id: l.TxnId })).slice(0, 5),
            emailed,
          };
          out.push(base);
          // Synthetic "emailed" event when the send falls inside the range —
          // mirrors the audit log's separate "Emailed Invoice…" rows.
          const dt = row.DeliveryInfo?.DeliveryTime || null;
          if (emailed && dt && dt >= fromIso && dt <= toIso) {
            out.push({ ...base, action: 'emailed', lastUpdated: dt, eventAt: dt });
          }
        }
        if (rows.length < PAGE) break;
        startPos += PAGE;
        if (page === MAX_PAGES - 1) capped.push(type);
      }
    } catch (e) {
      errors.push({ entity: type, error: e.message });
    }
    counts[type] = out.length;
    return out;
  }));

  // Tier 2 — deletes (CDC) + attachments (Attachable). Additive + best-effort:
  // a failure in either degrades to an error note, never breaks the main list.
  const [delRes, attRes] = await Promise.all([
    _fetchDeletes(list, fromIso, toIso),
    _fetchAttachments(fromIso, toIso),
  ]);
  if (delRes.skipped) notes.push('Deleted records show only for ranges within the last 30 days (QuickBooks CDC limit).');
  if (delRes.error)   errors.push({ entity: 'Deletes (CDC)', error: delRes.error });
  if (attRes.error)   errors.push({ entity: 'Attachments', error: attRes.error });
  counts.Deleted    = (delRes.deletes || []).length;
  counts.Attachment = (attRes.attachments || []).length;

  const rows = [...perEntity.flat(), ...(delRes.deletes || []), ...(attRes.attachments || [])]
    .sort((a, b) => String(b.eventAt || b.lastUpdated).localeCompare(String(a.eventAt || a.lastUpdated)));
  return { rows, counts, capped, errors, notes };
}
