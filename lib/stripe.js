...
async function createInvoiceForQuote({ quoteNumber, dealId, customer, lineItems, daysUntilDue = 7 }) {
  ...
  for (const item of itemsToCharge) {
    ...
    const cents = TO_CENTS(item.price);
    if (cents <= 0) continue;
    await stripeReq('/invoiceitems', {
      ...
      price_data: {
        ...
        product_data: { name: (item.name || 'Item').slice(0, 250) },
      },
