...
const FEATURE_SUBS = [
  ...
  { name: 'WA',
    ...
    resolve: (name, lineQty, ctx) => {
      ...
      // Narrow-wall shrink. WA STDDRFRM is 49" wide vs the std 46"/40" DRFRM,
      // so one adjacent narrow wall on the same long side has to flip to
      // absorb the door's extra width:
      //   • 46" wall booths:  C111 STDWL22 → C112 STDWL19 (3" shrink).
      //                       Inner: K112 IEPWL17.5 → K113 IEPWL14.5.
      //   • 40" wall booths:  C10  STDWL16 → C112 STDWL19 (3" change).
      //                       Inner: K09  IEPWL11.5 → K113 IEPWL14.5.
      // Geometry on 40" booths still TBD with Benton — wired per his note
      // ("the 16" wall would go to 19" wall") and we'll re-verify on tomorrow's
      // generated PLs.
      const narrow = wallSize === 46
        ? { stdOuter: 'C111', waOuter: 'C112', stdInner: 'K112', waInner: 'K113' }
        : { stdOuter: 'C10',  waOuter: 'C112', stdInner: 'K09',  waInner: 'K113' };
      if ((ctx.bomComponents[narrow.stdOuter] || 0) > 0) {
        for (let i = 0; i < lineQty; i++) {
      ...
      if (ctx.variant === 'E' || ctx.variant === 'ENV') {
        ...
        for (let i = 0; i < lineQty; i++) {
          ...
          if ((ctx.bomComponents[narrow.stdInner] || 0) > 0) {
