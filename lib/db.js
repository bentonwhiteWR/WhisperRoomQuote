...
async function initDb() {
  ...
  try {
    ...
    // Actual settlement date — null until HubSpot reports funds deposited
    // (the "Funds deposited to account ****X" event in their UI). Drives
    // the green "✓ Funds deposited" chip on deal cards + invoice rows.
    await db.query(`ALTER TABLE deal_payment_status ADD COLUMN IF NOT EXISTS actual_payout_date DATE`).catch(() => {});
    ...
    // Vendors catalog for the new WR PO System (supply-chain side, Josh's
    // home). Sits alongside supplier_pos which is Audimute-only (sales-rep
    // drop-ship). Catalog stores per-vendor parts (sku, description, mfg,
    // default qty/price, last-verified price date — old Excel POs carried
    // a "updated MM/DD/YYYY" stamp per line; we preserve that signal so
    // Josh can eyeball stale prices).
    //   contacts:       [{name}]      — internal display, who to address
    //   send_to_emails: [email]       — TO recipients on the mailto draft
    //   cc_emails:      [email]
    //   catalog: [{id, sku, description, mfg, mfg_part_no, default_qty, unit_price, price_updated_date}]
    await db.query(`
    ...
    `).catch(() => {});
    await db.query(`CREATE INDEX IF NOT EXISTS idx_vendors_name     ON vendors (lower(name))`).catch(() => {});
    await db.query(`CREATE INDEX IF NOT EXISTS idx_vendors_archived ON vendors (archived, lower(name))`).catch(() => {});

    // Vendor POs — the actual purchase orders Josh sends. Sits alongside
    // vendors (catalog). Separate table from supplier_pos (Audimute) by
    // design — the two systems have different lifecycles and ship-to
    // semantics. po_data snapshots the lines at creation time;
    // vendor_snapshot freezes the vendor block so historical POs aren't
    // mutated when Josh edits a vendor record later. received_data and
    // invoice_data are reserved for Phase 2/3 receive + invoice match
    // (added now so we don't need a migration when those land).
    //   status: DRAFT → APPROVED → SENT → PARTIAL → RECEIVED → CLOSED
    //           (+ CANCELLED reachable from any pre-RECEIVED state)
    //   po_data: { lines: [{itemId, sku, description, mfg, mfg_part_no,
    //             qty, unit_price}], notes, internal_notes }
    //   vendor_snapshot: full vendor row at time of creation (name,
    //             address, contacts, emails, freight/payment terms)
    await db.query(`
    ...
    `).catch(() => {});
    await db.query(`CREATE INDEX IF NOT EXISTS idx_vendor_pos_vendor ON vendor_pos (vendor_id, created_at DESC)`).catch(() => {});
    await db.query(`CREATE INDEX IF NOT EXISTS idx_vendor_pos_status ON vendor_pos (status, created_at DESC)`).catch(() => {});

    // ── Inventory (Packing-List migration Phase 2) ──
    // Replaces the Excel Inventory sheet + pCloud Inventory.csv round-trip.
    // One row per component code; on-hand is a single scalar (no bins/lots,
    // matching the Excel model). build_point is TEXT because the sheet uses
    // both numbers and the literal "as needed". Rows auto-create at 0 when a
    // ship deduction hits an unknown code (the Excel macros silently SKIPPED
    // unknown codes — invisible shrinkage; we surface them instead).
    await db.query(`
      CREATE TABLE IF NOT EXISTS components_inventory (
    ...
    // Append-only audit of every stock move. Every change to on_hand goes
    // through here (reasons: ship / unship / adjust / sync / seed) — the
    // "who/when/why" the CSV system never had. on_hand_after snapshots the
    // resulting level so the history reads without replaying deltas.
    await db.query(`
    ...
    `).catch(() => {});
    await db.query(`CREATE INDEX IF NOT EXISTS idx_invtx_code  ON inventory_transactions (code, created_at DESC)`).catch(() => {});
    await db.query(`CREATE INDEX IF NOT EXISTS idx_invtx_quote ON inventory_transactions (quote_number)`).catch(() => {});
    await db.query(`CREATE INDEX IF NOT EXISTS idx_invtx_at    ON inventory_transactions (created_at DESC)`).catch(() => {});
