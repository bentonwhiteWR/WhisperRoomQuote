...
// (YYYY-MM-DD, inclusive). Mirrors QB's "Expenses by Vendor Summary"
// report in the UI. accountingMethod = 'Accrual' (default) or 'Cash'.
async function fetchExpensesByVendorSummary({ startDate, endDate, accountingMethod = 'Accrual' } = {}) {
  return fetchReport('ExpensesByVendorSummary', {
...
// that rolled up into that vendor's total.
async function fetchExpensesByVendorDetail({ startDate, endDate, vendorId, accountingMethod = 'Accrual' } = {}) {
  return fetchReport('TransactionList', {
    start_date:        startDate,
    end_date:          endDate,
    accounting_method: accountingMethod,
    ...(vendorId ? { vendor: vendorId } : {}),
    // Bills + Cash purchases + Credit card purchases — same scope as the
    // summary report. CreditCardPayment intentionally omitted (it just
    // moves money between accounts, not a vendor expense).
    transaction_type:  'Bill,CashPurchase,CreditCardPurchase',
  });
}
