...
// Strip the "MDL " prefix so we can match against filename conventions
// that use just the size+variant. Example: filename "4848 S EFP.pdf"
// needs the needle "4848 S", not "MDL 4848 S".
function _stripMdlPrefix(model) {
  return String(model || '').replace(/^\s*MDL\s+/i, '').trim();
}
