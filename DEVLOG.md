...
## Current focus (2026-05-18 — Sales Goal report shipped to staging; tax-compliance thread still open)
...
- **v1.23.0** (this session) — Sales Goal report, new default sub-tab in /reports. 12-month moving avg of net revenue (Closed Won + Shipped, amount − tax, freight included) → publishes 100% goal = avg × 1.05, with 90% / 120% tier numbers driving the monthly salary bonus (5/10/15%, step, capped). Goal locks at month start. Hero + MTD progress bar + 12-month bar chart with moving-avg dotted line. Includes Shopify ecommerce-owned deals (49384873). New endpoint GET /api/reports/sales-goal, 5-min in-memory cache. **First-load test on staging needed** — verify the goal numbers roughly match user's mental model + check that legacy deals (back-calc from tax_rate) aren't pulling the average down in a weird way. Data-quality footer shows the HubSpot total_tax_amount vs back-calc split, so any anomalies should be visible.
- **v1.22.0 + v1.22.1** (2026-05-15) — TaxJar line-item mode + manual FL surtax cap. v1.22.1 applies FL §212.054 manually using TaxJar's state/local rate split. Expected to land at $552.00 on the Pinellas test case (W-1105142605). **STILL NOT verified on staging** — pulled into the bigger compliance conversation last session, then this Sales Goal work. Should verify before promoting either to main.
...
| 1.22.1  | 2026-05-15 | **FL discretionary surtax cap — manual application.** v1.22.0 assumed TaxJar would auto-apply FL §212.054 in line-item mode. Verified today against the Pinellas order: TaxJar still returned uncapped $558.93. So we now apply the cap server-side using TaxJar's state/local rate split. For FL line-item requests: state portion uncapped, county/special-district surtax only on first $5k of each individual item (per-unit post-discount price, capped at $5k, × qty). Components rounded per-jurisdiction before summing to match QB to the cent. Triggers only when `to_state === FL` AND mode=line_items AND local rate > 0. Other states with per-item caps (TN single-article, etc.) deferred — would need their own block when surfaced by a real order. |
| 1.22.0  | 2026-05-15 | **TaxJar line-item mode — quote tax now matches QB tax to the penny across every US jurisdiction with per-item rules.** FL Pinellas order with MDL 4848 S over $5k surfaced the bug: quote showed $558.93, QB invoiced $552.00. Gap was Florida\'s $5k discretionary surtax cap (FL Stat. §212.054(2)(b)) — county 1% only applies to first $5k of each tangible item. QB\'s AST handles natively because it sees line items; we were POSTing TaxJar a single aggregate `subtotal` so it could only return a flat blended 7% rate which we multiplied against the whole basis. Fix: client builds `lineItems[]` from positive-priced quote lines + per-line discount allocation, sends to /api/tax. Server forwards as TaxJar `line_items[]`. TaxJar now applies FL cap, TN single-article cap, MA clothing exemption, NY apparel rules, etc. natively — no per-state code on our end. Effective rate back-computed from returned amount ÷ taxable basis so client-side live recompute lands on the same penny. Aggregate-amount mode kept as fallback. |
