...
const FEATURE_SUBS = [
  ...
  // WA RAMP — 3-box ramp system (matches the ENT PL example: rows 26–28).
  // Each quote line adds Z62 + Z63 + Z64 as three separate component rows
  // (RAMP WITH ADAPTER + 2 MIDDLE RAMPS + 3 LOWER RAMPS = 6 ramp pieces in
  // 3 boxes). "RAMP SYS" / bare "RAMP" intentionally don't match yet —
  // those will be separate rules once verified against tomorrow's PLs.
  { name: 'WA RAMP',
