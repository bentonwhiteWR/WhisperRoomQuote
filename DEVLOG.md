...
- **v1.23.0 / .1 / .2** (this session) — Sales Goal report, new default sub-tab in /reports. 12-month moving avg of net revenue (Closed Won + Shipped, amount − tax, freight included) → publishes 100% goal = avg × 1.05, with 90% / 120% tier numbers driving the monthly bonus (5/10/15%, step, capped). Goal locks at month start. Hero + MTD progress bar + 12-month bar chart with moving-avg dotted line + deal counts per month. Includes Shopify ecommerce-owned deals (49384873). New endpoint GET /api/reports/sales-goal, 5-min in-memory cache.
