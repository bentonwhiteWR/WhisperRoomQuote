...
const FEATURE_SUBS = [
  ...
  { name: 'WA',
    ...
    resolve: (name, lineQty, ctx) => {
      ...
      // Narrow-wall shrink. WA STDDRFRM is 49" wide, so on 46"-wall booths
      // one adjacent narrow wall has to flip from C111 STDWL22 → C112 STDWL19
      // to absorb the doorframe's 3" extra width (46+22 = 49+19 = 68"). Inner
      // shell mirrors: K112 IEPWL17.5 → K113 IEPWL14.5.
      //
      // 40"-wall booths: NOT YET WIRED. Door grows 9" (40→49) but every
      // 40"-wall booth in base-bom.json carries STDWL16 (C10) as its only
      // narrow, and 16-9=7 has no clean swap. Real geometry TBD against a
      // CP-generated PL — possibilities: (a) the 16" wall gets removed
      // entirely and the WA doorframe + 7" remainder eat the slot, (b) the
      // booth physically extends, (c) a different wall absorbs the delta.
      // Skipping the swap until confirmed.
      ...
      if (ctx.variant === 'E' || ctx.variant === 'ENV') {
        ...
        for (let i = 0; i < lineQty; i++) {
          ...
          // 46" only — 40" inner geometry waits on the same confirmation as the
          // outer swap.
          if (narrow && (ctx.bomComponents[narrow.stdInner] || 0) > 0) {
