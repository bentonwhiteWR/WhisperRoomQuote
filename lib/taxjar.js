...
async function calculateTaxProper(toState, toZip, toCity, amount, shipping, toStreet = '', installShipping = 0, lineItems = null) {
  ...
  if (res.body && res.body.tax) {
    const taxAmt = res.body.tax.amount_to_collect || 0;
    // Back-compute an EFFECTIVE rate from the actual returned amount + the
    // taxable basis TaxJar saw. Client's getTaxAmount() does live recompute as
    // `rate × taxableBase` whenever products / freight / discount change, so
    // the rate we store has to be one that yields the correct dollar figure
    ...
    // For uncapped orders, effectiveRate ≈ headlineRate.
    let taxableBasis;
    if (body.line_items) {
