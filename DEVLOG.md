...
**On STAGING (NOT YET promoted to main):** v1.22.0 — TaxJar line-item mode. Quote tax now matches QB tax to the penny across every US jurisdiction with per-item rules (FL $5k cap, TN single-article cap, MA $175 clothing exemption, etc.). User to verify a few orders (FL, TN, NY, an exempt state, a non-nexus state) before promoting.
