...
async function fetchActivity({ fromIso, toIso, entities } = {}) {
      ...
      for (let page = 0; page < MAX_PAGES; page++) {
        ...
        for (const row of rows) {
          ...
          out.push({
            ...
            created,
            lastUpdated: row.MetaData?.LastUpdatedTime || null,
  ...
  const rows = perEntity.flat().sort((a, b) => String(b.lastUpdated).localeCompare(String(a.lastUpdated)));
  return { rows, counts, capped, errors };
}
