...
let HX_WA_EXT_SWAP = {};        // waType → { std:[{remove,add}], enh:[{remove,add}] } — HX+WA/ADA EXT-wall re-size
...
function init() {
  ...
  try {
    ...
    HX_WA_EXT_SWAP    = fr.hx_wa_ext_swap || {};
...
const FEATURE_SUBS = [
  ...
  { name: 'RM',
    ...
    resolve: (name, _lineQty, ctx) => {
      ...
      // Ceiling support beam — Standard (S) booths omit the IEP beam that E
      // booths carry; under Roof Mount the ceiling bears the roof-mount load, so
      // add it back: one U101 per D101 (96-series) / one U01 per D12 (102-series).
      // Count matches the E variant's beam count. E/SNV/ENV are unaffected.
      if (String(ctx.variant || '').toUpperCase() === 'S') {
        const bom = ctx.bomComponents || {};
        for (let i = 0; i < (bom.D101 || 0); i++) additions.push({ code: 'U101', fromFeature: name });
        for (let i = 0; i < (bom.D12  || 0); i++) additions.push({ code: 'U01',  fromFeature: name });
      }
...
function generate(lineItems, opts = {}) {
  ...
  const out = rooms.map((room, idx) => {
    ...
    if (resolved) {
      ...
      // A booth LINE with qty N is N identical booths, and its accessory lines
      // carry the ORDER TOTAL for the group (e.g. 4 rooms + "BASS TRAPS" qty 12).
      // Each booth's PL should get its share, so divide every feature qty by the
      // unit count before resolving (12 traps ÷ 4 booths = 3 per booth). Single-
      // booth lines (qty 1) are unchanged.
      const unitCount = Math.max(1, parseInt(room.qty, 10) || 1);
      const perBoothFeatures = unitCount > 1
        ? room.features.map(f => ({ ...f, qty: Math.max(1, Math.round((parseInt(f.qty, 10) || 1) / unitCount)) }))
        : room.features;
      const { remaining, additions, removals } = applyFeatureSubs(perBoothFeatures, ctx);
      ...
      // HX + WA-upgrade/ADA EXT re-size (per the spec chart). When a booth has
      // HX AND a WA upgrade or ADA, the height-extension EXT walls HX added at
      // the door swap to the WA-door EXT variants for the booth's WA Type. Std
      // rows always; IEP (enh) rows additionally on E/ENV booths. Each row pulls
      // one EXT code (added by HX) and adds the matching Z EXT code.
      const featNames = (room.features || []).map(f => String(f.name || ''));
      const hasHX  = featNames.some(n => /^HX\b/i.test(n));
      const hasWA  = featNames.some(n => /^WA\s+UPG\b/i.test(n));
      const hasADA = featNames.some(n => /^ADA\b/i.test(n));
      if (hasHX && (hasWA || hasADA)) {
        // WA Type: explicit WA-UPG wall size wins, else the booth's primary wall;
        // then the order's repWaType (ctx.adaWaType) overrides, else default from
        // the booth (narrow 4622/4016 only if it carries the 22"/16" wall).
        let wallSize = ctx.primaryWallSize;
        const waFeat = featNames.find(n => /^WA\s+UPG\b/i.test(n));
        const wm = waFeat && /^WA\s+UPG(?:\s+(?:STD|ENH))?(?:\s+(\d{2}))?/i.exec(waFeat);
        if (wm && wm[1]) wallSize = parseInt(wm[1], 10);
        let waType = null;
        if (wallSize === 46)      waType = (ctx.adaWaType === '4646' || ctx.adaWaType === '4622') ? ctx.adaWaType : ((ctx.bomComponents['C111'] || 0) > 0 ? '4622' : '4646');
        else if (wallSize === 40) waType = (ctx.adaWaType === '4040' || ctx.adaWaType === '4016') ? ctx.adaWaType : ((ctx.bomComponents['C10']  || 0) > 0 ? '4016' : '4040');
        const swap = waType && HX_WA_EXT_SWAP[waType];
        if (swap) {
          const isEnh = /^E/i.test(ctx.variant || '');   // E / ENV (has the IEP inner shell)
          const rows = (swap.std || []).concat(isEnh ? (swap.enh || []) : []);
          // Door-side toggle: a row may carry an `_alt` code (the mirror part).
          // The 4016 jamb adapter (Z19→Z20 T/R) flips to Z21 (T/L) when the door
          // is on the left — picker placement the rep confirms with the client.
          // Z20/Z21 weigh the same, so flipping is weight-neutral. Default = R.
          const leftJamb = /^L/i.test(String(opts.jambSide || ''));
          for (const row of rows) {
            const li = lines.findIndex(l => l.code === row.remove);
            if (li < 0) continue;   // HX didn't add this EXT wall on this booth — skip
            const addCode = (row._alt && leftJamb) ? row._alt : row.add;
            const removed = lines.splice(li, 1)[0];
            if (removed && removed.eachLb != null) netLb = (netLb || 0) - removed.eachLb;
            const c = COMPONENTS[addCode] || {};
            const eachLb = (c.lb == null) ? null : Number(c.lb);
            if (eachLb != null) netLb = (netLb || 0) + eachLb;
            lines.push({ code: addCode, desc: c.desc || '(unknown component)', pack: c.pack || '', eachLb,
              L: c.L ?? null, W: c.W ?? null, T: c.T ?? null, known: !!COMPONENTS[addCode], addedFrom: 'HX+WA' });
          }
        }
      }
