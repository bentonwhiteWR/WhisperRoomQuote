...
// Corporate INCOME-tax economic-nexus thresholds, distinct from the sales-tax
// thresholds above. Three shapes:
//   { receipts, txns?, note? } — a bright-line / factor-presence dollar trigger
//   { noTax: true, note? }     — state has no corporate income tax (some run a
//                                gross-receipts tax instead, noted)
//   { physical: true, note? }  — has a corp income tax but NO bright-line
//                                economic-nexus statute; nexus rests on physical
//                                presence / P.L. 86-272
// REFERENCE ONLY, ~2025–2026 — income-tax nexus is murkier than sales-tax and
// several states index annually. Confirm with the accountant before acting.
// Sources: state DORs, Wolters Kluwer factor-presence chart, CA FTB 2025 indexed
// amounts, NY Article 9-A deriving-receipts threshold.
const INCOME_NEXUS = {
  // Bright-line / factor-presence dollar thresholds
  AL:{receipts:538000, note:'factor, indexed'},
  CA:{receipts:757070, note:'factor, indexed'},
  CO:{receipts:500000, note:'factor'},
  CT:{receipts:500000, note:'bright-line'},
  HI:{receipts:100000, txns:200, note:'Wayfair'},
  MA:{receipts:500000, note:'factor'},
  MI:{receipts:350000, note:'factor'},
  NJ:{receipts:100000, txns:200, note:'CBT'},
  NY:{receipts:1283000, note:'deriving receipts'},
  PA:{receipts:500000, note:'factor (CNIT)'},
  TN:{receipts:500000, note:'factor (F&E tax)'},
  WV:{receipts:500000, note:'factor'},
  // No corporate income tax (gross-receipts tax noted where one exists)
  NV:{noTax:true, note:'Commerce Tax >$4M'},
  OH:{noTax:true, note:'CAT gross-receipts'},
  SD:{noTax:true},
  TX:{noTax:true, note:'franchise/margin tax'},
  WA:{noTax:true, note:'B&O gross-receipts'},
  WY:{noTax:true},
  // Corp income tax, but physical-presence / P.L. 86-272 (no bright-line $)
  AK:{physical:true}, AZ:{physical:true}, AR:{physical:true}, DE:{physical:true},
  DC:{physical:true}, FL:{physical:true}, GA:{physical:true}, ID:{physical:true},
  IL:{physical:true}, IN:{physical:true}, IA:{physical:true}, KS:{physical:true},
  KY:{physical:true}, LA:{physical:true}, ME:{physical:true}, MD:{physical:true},
  MN:{physical:true}, MS:{physical:true}, MO:{physical:true}, MT:{physical:true},
  NE:{physical:true}, NH:{physical:true, note:'Business Profits Tax'}, NM:{physical:true},
  NC:{physical:true}, ND:{physical:true}, OK:{physical:true},
  OR:{physical:true, note:'+ CAT $750k'}, RI:{physical:true}, SC:{physical:true},
  UT:{physical:true}, VT:{physical:true}, VA:{physical:true}, WI:{physical:true},
};

// Human-readable income-tax nexus label, e.g. "$500k factor", "$1.283M deriving
// receipts", "No corp income tax (CAT gross-receipts)", "Physical presence".
function incomeNexusLabel(stateAbbr) {
  const t = INCOME_NEXUS[String(stateAbbr || '').toUpperCase()];
  if (!t) return '';
  if (t.noTax)    return 'No corp income tax' + (t.note ? ` (${t.note})` : '');
  if (t.physical) return 'Physical presence'  + (t.note ? ` (${t.note})` : '');
  if (t.receipts) {
    const n = t.receipts;
    let dollars;
    if (n >= 1000000)      dollars = '$' + (n / 1000000).toFixed(3).replace(/\.?0+$/, '') + 'M';
    else if (n % 1000 === 0) dollars = '$' + (n / 1000) + 'k';
    else                   dollars = '$' + n.toLocaleString();
    let s = dollars;
    if (t.txns) s += ` or ${t.txns} txns`;
    if (t.note) s += ` ${t.note}`;
    return s;
  }
  return '';
