// Rep notifications (DB + email logging)
// Extracted from quote-server.js — named exports, no behavior changes.
// Host must call `init({ getDb })` before any notify function is invoked.

let _getDb;

function init(deps) {
  _getDb = deps.getDb;
}

// Rep email addresses — keyed by HubSpot owner ID. Used by:
//   (1) notification email-intent logging in notifyRep()
//   (2) session ownerId hydration fallback in quote-server.js when
//       HubSpot Owners API misses (lookups by real login email).
// Values MUST match each rep's actual login email — these are now
// load-bearing for the bell to render notifications. Mirror of the
// orders-dashboard.html REP_EMAILS map (source of truth).
const REP_EMAILS = {
  '36303670': 'bentonwhite@whisperroom.com',
  '38732178': 'accounting@whisperroom.com',
  '36330944': 'jholdway@whisperroom.com',
  '38143901': 'ssmith@whisperroom.com',
  '117442978':'tsingleton@whisperroom.com',
  '36320208': 'gabrielwhite@whisperroom.com',
  '38900892': 'cburgess@whisperroom.com',
  '38732186': 'shipping@whisperroom.com',
  '49384873': 'ecommerce@whisperroom.com',
};

async function createNotification(ownerId, type, title, body, { dealId, dealName, quoteNum } = {}) {
  const db = _getDb();
  // Log every call so Railway logs surface the trigger even when ownerId
  // is missing (silent no-ops were impossible to diagnose previously).
  if (!ownerId) {
    console.warn(`[notify] skipped — ownerId missing for type=${type} title="${title}" quoteNum=${quoteNum || ''}`);
    return;
  }
  if (!db) {
    console.warn(`[notify] skipped — no DB for type=${type} ownerId=${ownerId}`);
    return;
  }
  try {
    await db.query(
      `INSERT INTO notifications (owner_id, type, title, body, deal_id, deal_name, quote_num)
       VALUES ($1,$2,$3,$4,$5,$6,$7)`,
      [String(ownerId), type, title, body || null, dealId || null, dealName || null, quoteNum || null]
    );
    console.log(`[notify] created type=${type} ownerId=${ownerId} title="${title}"${quoteNum ? ' quoteNum=' + quoteNum : ''}`);
  } catch(e) { console.warn('[notify] DB insert failed:', e.message); }
}

async function notifyRep(ownerId, subject, bodyText, meta = {}) {
  if (!ownerId) return;
  // Create internal notification
  await createNotification(ownerId, meta.type || 'info', subject, bodyText, meta);
  // Log email intent (add SMTP later if needed)
  const email = REP_EMAILS[String(ownerId)];
  if (email) console.log(`[notify] ${email} — ${subject}`);
}

module.exports = {
  init,
  REP_EMAILS,
  createNotification,
  notifyRep,
};
