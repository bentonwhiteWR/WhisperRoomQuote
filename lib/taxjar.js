...
async function calculateTaxProper(toState, toZip, toCity, amount, shipping, toStreet = '', installShipping = 0) {
  ...
  // retry the request without `to_zip` (and without `to_street`,
  // which is meaningless without a ZIP). If a city is present TaxJar
  // returns a city/county rate; otherwise the state base rate. Marks
  // the result with usedStateFallback so the client can label it
  // and the rep knows to double-check the ZIP.
  ...
  if (isZipNotInState) {
    console.log(`[tax] ZIP ${toZip} rejected for ${stateUpper} — retrying without ZIP. (${detailMsg})`);
    const fallbackBody = { ...body };
    delete fallbackBody.to_zip;
    ...
    fallbackReason    = `ZIP ${toZip} not recognized — using ${body.to_city ? 'city/state' : 'state'}-level rate`;
  }
