...
async function createInvoice({ customerRef, docNumber, txnDate, lines, memo, billAddr, shipAddr, globalTaxCalc, shippingLine }) {
  const { accessToken, realmId } = await getAccessToken();
  const payload = {
    ...
    // ShippingLine is a top-level field (not inside the Line array).
    // It populates QB's dedicated Shipping totals row rather than the line items grid.
